...
let _file = null
function getFile() {
  if (!_file) _file = path.join(app.getPath('userData'), 'history.json')
  return _file
}
...
function load() {
  try {
    return JSON.parse(fs.readFileSync(getFile(), 'utf8'))
  } catch {
    return []
  }
}
...
function save(list) {
  ...
  fs.writeFileSync(getFile(), JSON.stringify(trimmed, null, 2), 'utf8')
}
